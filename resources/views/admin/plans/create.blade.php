    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.plans.store") }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('starting_balance') ? 'is-invalid' : '' }}" type="number" name="starting_balance" id="starting_balance" value="{{ old('starting_balance', '') }}" step="1" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('upgrade_threshold') ? 'is-invalid' : '' }}" type="number" name="upgrade_threshold" id="upgrade_threshold" value="{{ old('upgrade_threshold', '') }}" step="0.01" required>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('server') ? 'is-invalid' : '' }}" name="server_id" id="server_id" required>
                    @foreach($servers as $id => $entry)
                        <option value="{{ $id }}" {{ old('server_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('next_plan') ? 'is-invalid' : '' }}" type="number" name="next_plan" id="next_plan" value="{{ old('next_plan', '0') }}" step="1" required>
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('new_account_on_next_plan') ? 'is-invalid' : '' }}">
                    <input class="form-check-input" type="checkbox" name="new_account_on_next_plan" id="new_account_on_next_plan" value="1" required {{ old('new_account_on_next_plan', 0) == 1 ? 'checked' : '' }}>
