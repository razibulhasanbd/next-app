...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.modelVarient.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.model-varients.store") }}" enctype="multipart/form-data">
            @csrf
            ...
            <div class="form-group">
                ...
                @foreach(App\Models\ModelVarient::IS_DEFAULT_RADIO as $key => $label)
                    <div class="form-check {{ $errors->has('is_default') ? 'is-invalid' : '' }}">
                        <input class="form-check-input" type="radio" id="is_default_{{ $key }}" name="is_default" value="{{ $key }}" {{ old('is_default', '1') === (string) $key ? 'checked' : '' }} required>
