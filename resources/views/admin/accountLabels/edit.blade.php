...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.account-labels.update", [$accountLabel->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('account') ? 'is-invalid' : '' }}" name="account_id" id="account_id" required>
                    @foreach($accounts as $id => $entry)
                        <option value="{{ $id }}" {{ (old('account_id') ? old('account_id') : $accountLabel->account->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('labels') ? 'is-invalid' : '' }}" name="labels[]" id="labels" multiple required>
                    @foreach($labels as $id => $label)
                        <option value="{{ $id }}" {{ (in_array($id, old('labels', [])) || $accountLabel->labels->contains($id)) ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
