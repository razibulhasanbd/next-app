...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.extendCycleLog.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.extend-cycle-logs.store") }}" enctype="multipart/form-data">
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('login') ? 'is-invalid' : '' }}" name="login_id" id="login_id" required>
                    @foreach($logins as $id => $entry)
                        <option value="{{ $id }}" {{ old('login_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('login'))
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('subcription') ? 'is-invalid' : '' }}" name="subcription_id" id="subcription_id" required>
                    @foreach($subcriptions as $id => $entry)
                        <option value="{{ $id }}" {{ old('subcription_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('weeks') ? 'is-invalid' : '' }}" type="number" name="weeks" id="weeks" value="{{ old('weeks', '') }}" step="1" required>
                @if($errors->has('weeks'))
                    <div class="invalid-feedback">
                        {{ $errors->first('weeks') }}
                    </div>
            ...
            <div class="form-group">
                ...
                <input class="form-control datetime {{ $errors->has('before_subscription') ? 'is-invalid' : '' }}" type="text" name="before_subscription" id="before_subscription" value="{{ old('before_subscription') }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control datetime {{ $errors->has('after_subscription') ? 'is-invalid' : '' }}" type="text" name="after_subscription" id="after_subscription" value="{{ old('after_subscription') }}" required>
