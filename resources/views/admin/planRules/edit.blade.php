...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.planRule.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.plan-rules.update", [$planRule->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('rule_name') ? 'is-invalid' : '' }}" name="rule_name_id" id="rule_name_id" required>
                    @foreach($rule_names as $id => $entry)
                        <option value="{{ $id }}" {{ (old('rule_name_id') ? old('rule_name_id') : $planRule->ruleName->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('plan') ? 'is-invalid' : '' }}" name="plan_id" id="plan_id" required>
                    @foreach($plans as $id => $entry)
                        <option value="{{ $id }}" {{ (old('plan_id') ? old('plan_id') : $planRule->plan->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('plan'))
                    <div class="invalid-feedback">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('value') ? 'is-invalid' : '' }}" type="text" name="value" id="value" value="{{ old('value', $planRule->value) }}" required>
                @if($errors->has('value'))
                    <div class="invalid-feedback">
                        {{ $errors->first('value') }}
                    </div>
