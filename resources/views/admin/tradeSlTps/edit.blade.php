...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.tradeSlTp.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.trade-sl-tps.update", [$tradeSlTp->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('trade') ? 'is-invalid' : '' }}" name="trade_id" id="trade_id" required>
                    @foreach($trades as $id => $entry)
                        <option value="{{ $id }}" {{ (old('trade_id') ? old('trade_id') : $tradeSlTp->trade->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('trade'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('type') ? 'is-invalid' : '' }}" type="number" name="type" id="type" value="{{ old('type', $tradeSlTp->type) }}" step="1" required>
                @if($errors->has('type'))
                    <div class="invalid-feedback">
                        {{ $errors->first('type') }}
                    </div>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('value') ? 'is-invalid' : '' }}" type="text" name="value" id="value" value="{{ old('value', $tradeSlTp->value) }}" required>
                @if($errors->has('value'))
                    <div class="invalid-feedback">
                        {{ $errors->first('value') }}
                    </div>
