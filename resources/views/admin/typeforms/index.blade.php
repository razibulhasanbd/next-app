...
@section('content')
    ...
    <div class="card">
        <div class="card-header">
            {{ trans('cruds.typeform.title_singular') }} {{ trans('global.list') }}
        </div>

        <div class="row card-body">
            <form class="form-inline" method="POST" action="{{ route('admin.typeform.downloadOutsidePayment') }}" enctype="multipart/form-data">
                @csrf
                <div class="col-md-3">
    ...
    <div class="modal fade" id="transactionModalOpen" data-backdrop="static" data-keyboard="false" role="dialog"
         aria-labelledby="exampleModalCenterTitle" aria-hidden="true" :no-enforce-focus="true">
        <div class="modal-dialog" role="document" style="top: 35%;">
            <div class="modal-content">

                <div class="modal-body">
                    <h5>Previous transaction id : <span id="transaction_id" class="badge badge-primary"></span> </h5>
                    <input type="text" class="form-control" id="transactionModalInput">
                    <input type="hidden" id="transactionModalInputRow">
                </div>
                <div class="modal-footer">
                    <button type="button" class="btn btn-secondary" data-dismiss="modal"
                            onclick="modalClose()">Close</button>
                    <button type="button" class="btn btn-primary" onclick="transactionUpdate()"
                            data-dismiss="modal">Update</button>
                </div>
            </div>
        </div>
...
@section('scripts')
    ...
    <script>
           ...
           function updatePaymentStatus(val) {
            ...
            if (confirm('Are you sure you want to Change this?')) {
                ...
                if(status_id==0){
                    ...
                    $(tr).css('background-color', '#fae0de');
                }
                else if(status_id==3){
        ...
        function modalOptions(modalrow) {
           ...
           function transactionModalOptions(modalrow) {
               let transaction = document.getElementById('transaction' + modalrow).innerText;
               let modalInput = document.getElementById('transactionModalInput');
               modalInput.value = transaction;
               let modalInputRow = document.getElementById('transactionModalInputRow');
               modalInputRow.value = modalrow;
               document.getElementById('transaction_id').innerText = transaction;
           }
        ...
        function remarksModalOptions(modalrow) {
           ...
           function historyModalOptions(rowId) {
               ...
               $.ajax({
                   url: "{!! route('admin.typeforms.showHistory') !!}",
                   type: "POST",
                   data: {rowId: rowId},
                   dataType: "html",
                   beforeSend: function() {
                   ...
                   success: function (data) {
                       ...
                       var html_content
                       var show_history = 'show_history'+rowId
                       document.getElementById(show_history).innerHTML = 'Show History';
                   },
        ...
        function transactionUpdate() {
               let modalInput = document.getElementById('transactionModalInput').value;
               let rowId = document.getElementById('transactionModalInputRow').value;
               let modalInputRow = document.getElementById('transactionModalInputRow');
               $.ajax({
                   headers: {
                       'x-csrf-token': _token
                   },
                   url: '{{ route('admin.webhook.updateTransaction') }}',
                   method: 'post',
                   dataType: 'json',
                   data: {
                       "modalInput": modalInput,
                   ...
                   success: function(responseData) {
                       ...
                       modalInput.value = "";
                       modalInputRow.value = "";
                       let fn_amount = document.getElementById('transaction' + rowId);
                       fn_amount.innerHTML = "";
                       fn_amount.innerHTML = responseData.transaction_id;
                       rowId.value = "";
                   },
                   error: function(responseData) {
                       console.log(responseData);
        ...
        // function denyThisPayment(rowId){
        ...
        $(function() {
            ...
            @can('typeform_delete')
                ...
                let deleteButton = {
                    ...
                    url: "{{ route('admin.typeforms.massDestroy') }}",
            ...
            let dtOverrideGlobals = {
                ...
                scroller: true,
                aaSorting: [],
                ajax: "{{ route('admin.typeforms.index') }}",
                columns: [{
                        data: 'placeholder',
                        name: 'placeholder',
                    },
                    ...
                    },
                    {
                        ...
                        mRender: function(data, type, row) {
                            return '<h5><span class="badge badge-secondary" id="transaction'+row.id+'">'+ data +' </span></h5>'+"@can('outside_payment_transaction_id_update')" + '<button type="button" data-toggle="modal" data-target="#transactionModalOpen" class="badge badge-primary" onclick="transactionModalOptions('+row.id+')" >Update</button>'+ "@endcan";
                        }
                    },
                    {
                        data: 'paid_amount',
                    ...
                    },
                    {
                        ...
                        mRender: function(data, type, row) {
                            return '<span>'+ data +' </span>'+"@can('outside_payment_show_history')" + ' <button  id="show_history'+row.id+'" type="button" data-toggle="modal" data-target="#historyModalOpen" class="badge badge-primary" onclick="historyModalOptions('+row.id+')" >Show History</button>'+ "@endcan";
                        }
                    }
                ],
                orderCellsTop: true,
