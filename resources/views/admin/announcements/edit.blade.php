...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.announcements.update", [$announcement->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                <label for="news">{{ trans('cruds.announcement.fields.news') }}</label>
                <textarea class="form-control ckeditor {{ $errors->has('news') ? 'is-invalid' : '' }}" name="news" id="news">{!! old('news', $announcement->news) !!}</textarea>
                @if($errors->has('news'))
                    <div class="invalid-feedback">
                        {{ $errors->first('news') }}
                    </div>
                @endif
                <span class="help-block">{{ trans('cruds.announcement.fields.news_helper') }}</span>
            </div>
            <div class="form-group">
