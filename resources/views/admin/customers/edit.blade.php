    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.customers.update", [$customer->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('password') ? 'is-invalid' : '' }}" type="text" name="password" id="password" value="{{ old('password', $customer->password) }}" required>
            ...
            <div class="form-group">
                ...
                @foreach ($customerTags as $key => $label)
                    <div class="form-check {{ $errors->has('tags') ? 'is-invalid' : '' }}">
                        <input class="form-check-input" type="radio" name="tags"
                            value="{{ $key }}" {{ old('tags', $customer->tags  == $key) ? 'checked' : '' }}
                            required>
