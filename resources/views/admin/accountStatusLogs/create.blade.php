...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.accountStatusLog.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.account-status-logs.store") }}" enctype="multipart/form-data">
            @csrf
            ...
            <div class="form-group">
                ...
                <textarea class="form-control ckeditor {{ $errors->has('data') ? 'is-invalid' : '' }}" name="data" id="data">{!! old('data') !!}</textarea>
                @if($errors->has('data'))
                    <div class="invalid-feedback">
                        {{ $errors->first('data') }}
                    </div>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('old_status') ? 'is-invalid' : '' }}" name="old_status_id" id="old_status_id">
                    @foreach($old_statuses as $id => $entry)
                        <option value="{{ $id }}" {{ old('old_status_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('new_status') ? 'is-invalid' : '' }}" name="new_status_id" id="new_status_id" required>
                    @foreach($new_statuses as $id => $entry)
                        <option value="{{ $id }}" {{ old('new_status_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
