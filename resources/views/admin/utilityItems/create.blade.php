...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.utilityItem.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.utility-items.store") }}" enctype="multipart/form-data">
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('utility_category') ? 'is-invalid' : '' }}" name="utility_category_id" id="utility_category_id" onchange=utilityCategory(this.value) required>
                    @foreach($utility_categories as $id => $entry)
                        <option value="{{ $id }}" {{ old('utility_category_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group" id="download_file_url_div">
                ...
                <input class="form-control {{ $errors->has('download_file_url') ? 'is-invalid' : '' }}" type="file" name="download_file_url" id="download_file_url" >
            ...
            <div class="form-group" id="youtube_embedded_url_div">
                ...
                <textarea class="form-control {{ $errors->has('youtube_embedded_url') ? 'is-invalid' : '' }}" name="youtube_embedded_url" id="youtube_embedded_url" onchange="youtubeThumbnail()">{{ old('youtube_embedded_url') }}</textarea>
                <div class="mt-2">
                    <label for="youtube_thumbnail_url">{{ trans('cruds.utilityItem.fields.youtube_thumbnail_url') }}</label>
                    <input class="form-control mt-3 {{ $errors->has('youtube_thumbnail_url') ? 'is-invalid' : '' }}" type="text" name="youtube_thumbnail_url" id="youtube_thumbnail_url" value="{{ old('youtube_thumbnail_url', '') }}">
                    <img class="mt-3" id="preview_thumbnail" src="{{ old('youtube_thumbnail_url', '') }}" alt="Thumbnail Preview" width="200">
                </div>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('order_value') ? 'is-invalid' : '' }}" type="number" name="order_value" id="order_value" value="{{ old('order_value', '1') }}" step="1">
