...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.ruleName.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.rule-names.update", [$ruleName->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label class="required" for="name">{{ trans('cruds.ruleName.fields.name') }}</label>
                <input class="form-control {{ $errors->has('name') ? 'is-invalid' : '' }}" type="text" name="name" id="name" value="{{ old('name', $ruleName->name) }}" required>
                @if($errors->has('name'))
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('is_percent') ? 'is-invalid' : '' }}">
                    ...
                    <input class="form-check-input" type="checkbox" name="is_percent" id="is_percent" value="1" {{ $ruleName->is_percent || old('is_percent', 0) === 1 ? 'checked' : '' }}>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('condition') ? 'is-invalid' : '' }}" type="text" name="condition" id="condition" value="{{ old('condition', $ruleName->condition) }}" required>
