    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.products.store") }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('business_model') ? 'is-invalid' : '' }}" name="business_model_id" id="business_model_id" required>
                    @foreach($business_models as $id => $entry)
                        <option value="{{ $id }}" {{ old('business_model_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('model_varient') ? 'is-invalid' : '' }}" name="model_varient_id" id="model_varient_id" required>
                    @foreach($model_varients as $id => $entry)
                        <option value="{{ $id }}" {{ old('model_varient_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('buy_price') ? 'is-invalid' : '' }}" type="number" name="buy_price" id="buy_price" value="{{ old('buy_price', '') }}" step="0.01" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('topup_price') ? 'is-invalid' : '' }}" type="number" name="topup_price" id="topup_price" value="{{ old('topup_price', '') }}" step="0.01" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('reset_price') ? 'is-invalid' : '' }}" type="number" name="reset_price" id="reset_price" value="{{ old('reset_price', '') }}" step="0.01" required>
